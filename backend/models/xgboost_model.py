...
def create_features(df, lookback=5):
    ...
    data['rolling_max_5'] = data['close'].rolling(window=5).max()
    data['rolling_min_5'] = data['close'].rolling(window=5).min()
...
def xgboost_predict(df, lookback=5):
    """
    ...
    try:
        ...
        return {
            ...
            'metadata': {
                'name': 'XGBoost',
                'description': f'Extreme Gradient Boosting model using {lookback} lagged features and rolling statistics. High-performance gradient boosting.',
                'parameters': {
                    'n_estimators': 100,
                    'max_depth': 7,
                    'learning_rate': 0.1,
