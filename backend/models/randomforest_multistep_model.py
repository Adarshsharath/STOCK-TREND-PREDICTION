"""
Multi-Step Random Forest Model for Stock Price Forecasting
Predicts 5-7 days ahead with trend classification
Uses multi-output Random Forest regression
"""
...
def randomforest_multistep_predict(df, forecast_horizon=7, lookback=60):
    """
    Multi-step Random Forest prediction with trend classification
    ...
    try:
        ...
        # Split data: 80% train, 20% test (more training data)
        ...
        # OPTIMIZATION: Feature Selection using a smaller RF model
        selector_model = RandomForestRegressor(n_estimators=100, max_depth=10, random_state=42, n_jobs=-1)
        selector_model.fit(X_train_full, y_train)
        
        # Get importances and select top features (e.g., top 100 flattened features)
        ...
        # Train FINAL multi-output Random Forest with OPTIMIZED parameters
        model = RandomForestRegressor(
            ...
            n_jobs=-1,
            bootstrap=True,
            max_features='sqrt' # Better for high-dimensional sequence data
        )
        ...
        final_predictions = model.predict(last_features_selected)[0]
        ...
        test_start_idx = lookback + train_size
        ...
        return {
            ...
            'metadata': {
                'name': 'Multi-Step Random Forest (Optimized)',
                'description': f'Optimized Random Forest with feature selection predicting {forecast_horizon} days ahead',
                'parameters': {
                    'n_estimators': 300,
                    'max_depth': 20,
                    'features_kept': 100,
        ...
        raise Exception(f"Multi-step Random Forest prediction error: {str(e)}")
    """
    Multi-step Random Forest prediction with trend classification
    ...
    try:
        ...
        final_predictions = model.predict(last_features)[0]
        ...
        return {
            ...
            'metadata': {
                'name': 'Multi-Step Random Forest',
                'description': f'Random Forest model predicting {forecast_horizon} days ahead with trend classification',
                'parameters': {
                    'n_estimators': 200,
                    'max_depth': 15,
                    'lookback': lookback,
